...
:::note[如果执行 git pull 提示 error，请按照提示自行处理]
...
:::note[如果你的代码不是从 github 仓库获取的，请通过这里的隐藏帮助来进行迁移]
...
:::note[确保已经载入 py3 虚拟环境，中间如果遇到报错一般是依赖包没装全，可以通过 搜索引擎 解决]
...
:::note[如果执行 sh make_migrations.sh 有错误，请参考此处进行处理]
...
- 请接着根据 [1.4.4 升级到 1.4.5](1.4.4) 的文档升级到1.4.5，否则无法正常使用
:::
