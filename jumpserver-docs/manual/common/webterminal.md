---
title: Web 终端
description: Web 终端是普通用户连接使用被纳管资产的工作台，无需安装客户端即可通过浏览器登录服务器；本文介绍它的作用、适用场景与入口位置。
---
...
- 日常运维需要登录被纳管的服务器执行命令时；
- 需要连接被授权的数据库、Web 应用等资产时（具体支持范围以实际授权为准）；
- 在无法安装客户端的设备或临时办公环境中，需要应急登录目标资产时。

## 3 入口位置

Web 终端的入口位置如下图所示，首次进入有简单的使用指引。
