...
**系统消息** 是 JumpServer 面向当前登录用户的站内信通道。用户登录提醒、工单审批结果、危险命令告警等与个人相关的系统事件，都会以消息的形式推送到顶部导航栏的消息入口，用户无需守着某个页面等待，也不会错过与自己有关的处理结果。

## 1 功能简介

消息在顶部导航栏的消息入口集中展示，列表中包含消息标题、消息内容和接收时间，未读消息以醒目标识区分，进入页面即可看出是否有待处理的消息。

除站内信外，系统消息还支持邮件、企业微信、钉钉等通知渠道：渠道本身由管理员在 **系统设置 > 通知设置** 中配置，用户在 **个人设置 > 个人信息 > 通知设置** 中选择订阅哪些渠道；开启后同一事件可以同时通过站内信与外部渠道送达，便于及时响应。
...
浏览完消息后，可以点击 **全部已读** 将当前所有未读消息一次性标记为已读，保持列表清爽。消息由系统按事件自动产生，不能手动新建或删除，如需减少某类通知，可在个人设置的通知设置中调整对应的通知渠道。
