...
**远程应用** 指由管理员预先发布的远程应用，例如数据库连接中发布的 DBeaver、Web 资产中内置的 JumpServer WebLite。该类方式需要管理员先部署应用发布机并在其上发布应用，连接配置中才会出现对应选项，详见 [远程应用](../../admin/app_management/remote_apps.md)。远程应用的连接方式可在 **高级选项** 的 **远程应用连接方式** 中设置。

**虚拟应用** 指由管理员通过虚拟应用功能发布的 Linux 图形化应用，可用于发布 Web、数据库等资产，列表中可选的项即管理员已发布的虚拟应用名称。该方式需要管理员先在 **系统设置 > 功能设置 > 虚拟应用** 中开启该功能并完成发布，连接配置中才会出现对应选项，详见 [虚拟应用](../../admin/app_management/virtual_apps.md)。
