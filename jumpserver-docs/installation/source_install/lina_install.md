...
```bash
cd /opt
mkdir /opt/lina-v4.10.17
wget -O /opt/lina-v4.10.17.tar.gz https://github.com/jumpserver/lina/archive/refs/tags/v4.10.17.tar.gz
tar -xf lina-v4.10.17.tar.gz -C /opt/lina-v4.10.17 --strip-components 1
```
...
```
        ...
        ```bash
        sed -i "s@Version <strong>.*</strong>@Version <strong></strong>@g" src/layout/components/Footer/index.vue
        vi .env.development
        ```
        ```yaml
        # 全局环境变量 请勿随意改动
...
| OS     | Arch  | Name                                                          |
| :----- | :---- | :------------------------------------------------------------ |
| All    | All   | [lina-v4.10.17.tar.gz][lina-v4.10.17] |

```bash
cd /opt
wget https://github.com/jumpserver/lina/releases/download/v4.10.17/lina-v4.10.17.tar.gz
tar -xf lina-v4.10.17.tar.gz
```
