...
- **v3 升级 v4**：需先将当前版本升级到 v3 的最新版本（v3.10.x LTS），再升级到 v4，否则会因为表结构变更导致升级失败。
- **v4 升级 v5**：v4 到 v5 的数据库迁移是连续可前滚的，v4 各版本可直接升级到 v5，无需额外前置操作。若当前为较早期的 v4.0.0 版本，建议先升级到 v4 的最新版本（v4.10.x LTS）再升级到 v5。
