---
title: 网关设置
description: 介绍 1Panel AI 网关（Gateway）网关设置页的基础设置、性能设置、向量服务、智能路由、内容合规与 Elasticsearch 六个标签。
slug: /user_manual/ai_gateway/settings
---

## 1 概述

网关设置页用于配置网关的运行参数，页面上方以标签页形式划分为 **基础设置**、**性能设置**、**向量服务**、**智能路由**、**内容合规**、**Elasticsearch** 六个标签，单击对应标签即可切换。**网关设置**页面将这些全局配置集中在一个入口，是网关的"总控台"。平台级的企业账号登录配置（企业认证）位于「**系统设置**」页面，参见[系统设置](./system_settings)。
