...
**版本号命名规则**：1Panel AI 网关版本号格式为：v 大版本.功能版本.Bug 修复版本，示例如下：
...
- 新增多因素认证（MFA）功能
- 支持为上游账号配置代理
- `/v1/models` 接口支持返回全部模型
- 支持批量删除敏感词

**功能优化**

- 优化 Elasticsearch 对接时的错误提示
- 完善管理端的无障碍支持与界面一致性

**问题修复**

- 修复了多模态向量接口请求失败的问题
- 修复了 Hermes 提前断开连接时请求日志异常的问题
- 修复了 DeepSeek 接口请求失败的问题
- 修复了系统升级时报错的问题
...
- 增加钉钉、企业微信、飞书、LDAP 和 OIDC 登录支持

**功能优化**

- Elasticsearch 地址支持使用容器地址
- 统一网关消息大小限制
- 优化管理端页面布局和错误诊断
- 优化 Elasticsearch 账号保存失败时的错误提示
- 优化内容审核和智能路由的错误提示

**问题修复**

- 修复了火山引擎方舟向量模型导入失败的问题
- 修复了部分场景下 token 未记录的问题
...
- 【概览】新增反馈入口，便于提交问题与建议
- 【用户管理】支持一键重置用户 Token 用量
- 【概览】新增网关与上游账号的并发记录
- 【账号池】账号池新增测试功能

**功能优化**

- 【模型广场】优化模型广场的页面样式与展示效果
- 【账号池】优化模型账号导入大量模型时的处理逻辑

**问题修复**

- 【账号池】修复了 OpenCode Go 账号验证失败的问题
- 【模型组】修复了模型组选择过多模型时显示异常的问题
- 【模型代理】修复了删除上游账号后，相关模型仍出现在 `/v1/models` 返回结果中的问题
- 【模型代理】修复了 Claude Code Beta 版本请求失败的问题
- 【用户管理】修复部分场景下用户删除失败的问题
- 【账号池】修复了编辑模型账号时发现模型失败的问题
- 【调用日志】修复了调用日志列显示设置不生效的问题

相关链接：[GitHub Release](https://github.com/1Panel-dev/1Panel-Gateway/releases/tag/v1.0.2)
