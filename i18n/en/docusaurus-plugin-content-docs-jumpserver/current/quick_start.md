...
- Refer to [Linux standalone installation and deployment guide](installation/setup_linux_standalone/offline_install) for quick JumpServer deployment
:::

:::note[After successful installation, access and log in to JumpServer via browser]
...
- MySQL application requires granting `Core` and `KoKo` remote access permissions.
:::
