...
:::note[JumpServer Reverse Proxy Requirements]
...
- Follow recommendations from [Mozilla SSL Configuration Generator](https://ssl-config.mozilla.org/).
:::

## 1 Nginx SSL Deployment

:::note[Prepare SSL certificates (note: certificates must be in pem format)]
...
:::note[Recommend deploying SSL for more secure HTTPS protocol access]
