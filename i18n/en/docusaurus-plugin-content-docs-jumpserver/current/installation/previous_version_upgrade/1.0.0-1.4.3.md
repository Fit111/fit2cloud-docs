---
title: Upgrade from Version 1.0.0-1.4.3 to 1.4.4
---
...
:::note[If executing git pull prompts an error, handle according to the prompt]
...
:::note[If your code is not obtained from the GitHub repository, use this hidden help for migration]
...
:::note[Ensure Python3 virtual environment is loaded. If errors occur during installation, usually it's because dependency packages are not fully installed. Use search engines to solve]
...
:::note[If executing sh make_migrations.sh has errors, refer here for handling]
...
- Then upgrade according to the [1.4.4 to 1.4.5 upgrade](1.4.4.en) documentation, otherwise normal use will not be possible.
:::
